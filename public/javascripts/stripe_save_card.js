...
function show_saved_card(details) {
    ...
    let delete_card_button = $('<div class="float-right"> <button class="btn btn-info" onclick="click()" id="delete_saved_card" >Delete Card</button></div><br>')
...
function stripe_functionalities() {
    ...
    let saved_card_delete_button = $("#delete_saved_card");
    saved_card_delete_button.on("click", function(e) {
        e.preventDefault();

        let xhr = $.ajax('/payment/delete-card', {
            type: 'GET',
            crossDomain: false,
            headers: {
                'CSRF-Token': $('meta[name="csrf-token"]').attr('content')
        ...
        xhr.done(function(response) {
            toastr.info("Your card is removed successfully!");
        });

        xhr.fail(function(response) {
            toastr.error("An error occured when deleting saved card");
        })
    });
